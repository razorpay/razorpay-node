...
describe("PAYMENTS", () => {
  ...
  describe("Payment capture", () => {
    ...
    it("Payment capture request", (done) => {
      ...
      rzpInstance.payments
        .capture(paymentId, captureAmount, currency)
        .then((response) => {
          assert.equal(
            response.__JUST_FOR_TESTS__.url,
    ...
    it("Reject the promise if payment id is not provided", (done) => {
      rzpInstance.payments.bankTransfer().then(
        () => {
            ...
            new Error(`method bankTransfer does not check` + ` for payment_id`)
          );
        },
        () => {
          done();
  ...
  describe("Fetch card details with paymentId", () => {
    it("Reject the promise if payment id is not provided", (done) => {
      rzpInstance.payments.fetchCardDetails().then(
        () => {
          done(
            new Error(
              `method fetchCardDetails does not check` + ` for payment_id`
            )
          );
        },
        () => {
  ...
  describe("Payment Downtime ", () => {
    it("Fetch Payment Downtime Details", (done) => {
      ...
      rzpInstance.payments.fetchPaymentDowntime().then((response) => {
        assert.ok(
          equal(response.__JUST_FOR_TESTS__.url, "/v1/payments/downtimes")
        );
        done();
      });
    });
  ...
  describe("Fetch Payment Downtime", () => {
    it("Reject the promise if downtime id is not provided", (done) => {
      rzpInstance.payments.fetchPaymentDowntimeById().then(
        () => {
          done(
            new Error(
              `method fetchCardDetails does not check` + ` for payment_id`
  ...
  describe("Fetch a Specific Refund for a Payment", () => {
    it("Fetch Refund", (done) => {
      ...
      rzpInstance.payments
        .fetchRefund(TEST_PAYMENT_ID, TEST_REFUND_ID)
        .then((response) => {
          assert.ok(
            equal(
              response.__JUST_FOR_TESTS__.url,
  ...
  describe("Server to server json v2", () => {
    it("Create Payment Json", (done) => {
      ...
      rzpInstance.payments.createPaymentJson(params).then((response) => {
        assert.ok(
          equal(response.__JUST_FOR_TESTS__.url, "/v1/payments/create/json")
        );
        done();
      });
    });
