...
describe('PRODUCTS', () => {
  it('Create an account', (done) => {
    ...
    rzpInstance.products.requestProductConfiguration(TEST_ACCOUNT_ID, mockRequest).then((response) => {
      assert.equal(
  ...
  it('Update a Product Configuration', (done) => {
    ...
    rzpInstance.products.edit(TEST_ACCOUNT_ID, TEST_PRODUCT_ID, mockRequest).then((response) => {
        assert.equal(
        response.__JUST_FOR_TESTS__.url,
