...
const runIDRequiredTest = (params) => {
  ...
  it(`method ${methodName} checks for Invoice ID as param`, (done) => {
    apiObj[methodName](...methodArgs).then(
      () => {
        done(
          new Error(`method ${methodName} does not` + ` check for Invoice ID`)
        );
      },
      (err) => {
        done();
...
describe("INVOICES", () => {
  ...
  describe("Notify", () => {
    ...
    it("notify method checks for `medium` parameter", (done) => {
      ...
      apiObj[methodName](TEST_INVOICE_ID, undefined)
        .then(() => {
