...
const mocker = require("../../test/mocker"),
  ...
  chai = require("chai"),
  { assert } = chai,
  { prettify, getTestError } = require("../../dist/utils/razorpay-utils");
    ...
    apiObj[methodName](...methodArgs).then(
      (resp) => {
        const respData = resp.__JUST_FOR_TESTS__,
          respParams =
            respData[
              respData.method === "GET" ? "requestQueryParams" : "requestBody"
            ];
        const parseRespParams = JSON.parse(respParams);

        if (equal(parseRespParams, expectedParams)) {
