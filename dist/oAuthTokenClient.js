...
const {
  validateInput,
  SCHEMAS
} = require('./resources/oAuthTokenValidator');
class OAuthTokenClient extends API {
  ...
  generateAuthUrl(params) {
    ...
    const queryString = Object.entries(params).flatMap(([key, value]) => Array.isArray(value) ? value.map(item => `${key}[]=${encodeURIComponent(item)}`) : key === "redirect_uri" ? `${key}=${value}` : `${key}=${encodeURIComponent(value)}`).join("&");
